...
(function() {
  ...
  UserState = (function() {
    function UserState(conn) {
      ...
      this.set_ui_hidden = __bind(this.set_ui_hidden, this);
      this.set_ui_shown = __bind(this.set_ui_shown, this);
      ...
      this.user_session_state_el = $('.chat-container .user');
      this.nav_login_logout = $('.nav.login-logout');
      this.user_session_state_el.insertBefore(this.nav_login_logout).wrap('<nav class="nav nav-chat"></nav>');
      this.btn_chat_toggle = $('.chat-toggle');
      this.btn_chat_toggle.on({
        'click': function() {
          if ($('.chat-container').css('display') === 'none') {
            _this.conn.emit('req_user_ui_show');
          } else {
            _this.conn.emit('req_user_ui_hide');
          }
          return $('.chat-container').toggle();
        }
      });
    }
    ...
    UserState.prototype.set_available = function() {
      $('.chat-window').show();
      this.session_state_el.find('.state').html('Beschikbaar');
    ...
    UserState.prototype.set_busy = function() {
      $('.chat-window').show();
      this.session_state_el.find('.state').html('Niet storen');
      this.clear_btn_classes();
    ...
    UserState.prototype.set_invisible = function() {
      $('.chat-window').show();
      this.session_state_el.find('.state').html('Onzichtbaar');
      this.clear_btn_classes();
    ...
    UserState.prototype.set_ui_shown = function() {
      return $('.chat-container').show();
    };

    UserState.prototype.set_ui_hidden = function() {
      return $('.chat-container').hide();
    };
  ...
  Chat = (function() {
    function Chat(conn, chat_session, chat, user_list) {
      ...
      this.set_total_unread_messages = __bind(this.set_total_unread_messages, this);
    ...
    Chat.prototype.set_unread_messages = function(count) {
      ...
      if (count > 0) {
        unread_messages.html(count).addClass('active');
      } else {
        unread_messages.html('').removeClass('active');
      }
      return this.set_total_unread_messages();
    };

    Chat.prototype.set_total_unread_messages = function() {
      var total_unread_messages, unread_messages, unread_messages_el, unread_messages_els, _i, _len;
      unread_messages_els = $('.chat-list').find('.unread-messages');
      total_unread_messages = 0;
      for (_i = 0, _len = unread_messages_els.length; _i < _len; _i++) {
        unread_messages_el = unread_messages_els[_i];
        unread_messages = parseInt($(unread_messages_el).html(), 10);
        if (unread_messages) {
          total_unread_messages += unread_messages;
        }
      }
      return $('.total-unread-messages').html(total_unread_messages || '');
    };
  ...
  ChatApp = (function() {
    ...
    ChatApp.prototype.connect = function() {
      ...
      this.conn.on('ev_data_update', function(chat_session, chat_users, chats) {
        ...
        if (_this.chat_session.ui_is_visible === true) {
          _this.user_state.set_ui_shown();
        } else {
          _this.user_state.set_ui_hidden();
        }
